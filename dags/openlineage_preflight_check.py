...
def validate_ol_installation() -> None:
    ...
    if Version(airflow_version) >= Version("2.10.0") and library_version < Version(
        "1.8.0"
    ):
        raise ValueError(
    ...
    if latest_version is None:
        log.warning(
            f"Failed to fetch the latest version for `{library_name}`. Skipping version check."
        )
        return
...
def _check_http_env_vars() -> None:
    ...
    final_url = urljoin(
        os.getenv("OPENLINEAGE_URL", ""), os.getenv("OPENLINEAGE_ENDPOINT", "")
    )
    if final_url:
        raise ValueError(
            "OPENLINEAGE_URL and OPENLINEAGE_ENDPOINT are set to: %s", final_url
        )
    log.info(
    ...
    if _provider_can_be_used():
        try:
            from airflow.providers.openlineage.plugins.openlineage import (
                OpenLineageProviderPlugin as plugin,
            )
...
def _is_ol_disabled():
    if _provider_can_be_used():
        try:
            ...
            # apache-airflow-providers-openlineage < 1.7.0
            from airflow.providers.openlineage.plugins.openlineage import (
                _is_disabled as is_disabled,
            )
    else:
        from openlineage.airflow.plugin import _is_disabled as is_disabled

    if is_disabled():
        if _provider_can_be_used() and conf.getboolean(
            "openlineage", "disabled", fallback=False
        ):
            raise ValueError(
...
def _get_transport():
    if _provider_can_be_used():
        ...
        transport = (
            OpenLineageProviderPlugin()
            .listeners[0]
            .adapter.get_or_create_openlineage_client()
            .transport
        )
    else:
...
def _verify_marquez_http_backend(config):
    ...
    try:
        ...
        raise ConnectionError(
            f"Failed to connect to Marquez at `{list_namespace_url}`"
        ) from e
...
def openlineage_preflight_check_dag():
    @task(task_id="validate_ol_installation")
    def validate_ol_installation_task():
        validate_ol_installation()

    @task(task_id="is_ol_accessible_and_enabled")
    def is_ol_accessible_and_enabled_task():
        is_ol_accessible_and_enabled()

    @task(task_id="validate_connection")
    def validate_connection_task():
        validate_connection()

    (
        validate_ol_installation_task()
        >> is_ol_accessible_and_enabled_task()
